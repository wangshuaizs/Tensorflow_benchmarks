...
flags.DEFINE_boolean('allow_soft_placement', True,
                     'Whether to allow soft placement.')
...
class BenchmarkCNN(object):
  ...
  def _create_done_queue(self, i):
    """Queue used to signal death for i'th ps shard. Intended to have 
    all workers enqueue an item onto it to signal doneness."""
    print(i)
    with tf.device("/job:ps/replica:0/task:%d" % (i)):
      return tf.FIFOQueue(self.cluster_manager.num_workers(), tf.int32, shared_name="done_queue"+str(i))

  def _create_done_queues(self):
    return [self._create_done_queue(i) for i in range(self.cluster_manager.num_ps())]
  ...
  def run(self):
    ...
    if self.params.job_name == 'ps':
      ...
      sess = tf.Session(self.cluster_manager.get_target())
      queue = self._create_done_queue(self.task_index)
      # wait until all workers are done
      for i in range(self.cluster_manager.num_workers()):
